...
struct EmployeeDataRepository : BaseRepository
{
    ...
    func receiveAllEmployees() -> [Employee]? {
        guard let result = PersistentStorage.shared.fetchManagedObject(managedObject: CDEmployee.self) else {
            return nil
        }
    ...
    func receiveEmployeebyID(by id: UUID) -> CDEmployee? {
